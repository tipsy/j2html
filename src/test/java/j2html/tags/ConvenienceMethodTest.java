...
public class ConvenienceMethodTest {
  ...
  public void testAllConvenienceMethods() throws Exception {
    ...
    assertThat(
        a().withHref("http://example.com").render(), is("<a href=\"http://example.com\"></a>"));
    assertThat(div().withId("test-id").render(), is("<div id=\"test-id\"></div>"));
    assertThat(
        div().withData("testdata", "test").render(), is("<div data-testdata=\"test\"></div>"));
    ...
    assertThat(
        input().withPlaceholder("test-placeholder").render(),
        is("<input placeholder=\"test-placeholder\">"));
    ...
    assertThat(
        div().withStyle("background:red;").render(), is("<div style=\"background:red;\"></div>"));
