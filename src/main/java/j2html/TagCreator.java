...
public class TagCreator {
  ...
  /**
   * Returns a Attr.ShortForm object with either id, classes or both, obtained from parsing the
   * input string using css selector syntax
   *
  ...
  /**
   * Returns the HTML created by concatenating the input elements, separated by space, in encounter
   * order. Also removes spaces before periods and commas.
   *
  ...
  /**
   * Creates a DomContent object containing HTML using a mapping function on a collection Intended
   * usage: {@literal each(numbers, n -> li(n.toString()))}
   *
   * @param <T> The derived generic parameter type
   ...
   */
  public static <T> DomContent each(
      Collection<T> collection, Function<? super T, DomContent> mapper) {
    return rawHtml(
        collection.stream().map(mapper.andThen(DomContent::render)).collect(Collectors.joining()));
  }

  /**
   * Filters a collection to a list, to be used with {@link j2html.TagCreator#each} Intended usage:
